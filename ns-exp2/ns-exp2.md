...
- Impact 是由 CIA 决定，简单计算一下取值，最大为 **10.00084536 (10.41 x (1 - 0.34 ^ 3))**，最小值为 0。
- Exploitability 是由 AccessComplexity，Authentication，AccessVector组成，最大值为 **9.9968(20 x 0.71 x 0.704)**
...
**问题[1]解决**:不知道为什么，在CVSS v3 Calculator 网站上点击 "Show Equations"，显示出来的页面却是 CVSS v2 版本的计算公式，导致实验结果与公式分析结果的不符，这里重新提供 CVSS v3 的计算公式：[click here](https://nvd.nist.gov/vuln-metrics/cvss/v3-calculator)之后，点击"Show Equations"，这一部分应该是补充的新增规则，只要将开始的公式加上新增规则即可解决问题[1]。这里就不再具体分析。
