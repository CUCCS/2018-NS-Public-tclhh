...
> **实验输入各种指令，部分结果图片以指令命名。**



首先，了解一下最重要的管理帧的种类以及大致作用：
...
> **Beacon frame** is one of the management frames in [IEEE 802.11](https://en.wikipedia.org/wiki/IEEE_802.11) based WLANs. It contains all the information about the network. Beacon frames are transmitted periodically, **they serve to announce the presence of a wireless LAN and to synchronise the members of the service set**. 
>
...
如上图，我只能将 ESSID 为空作为隐藏 SSID 的特征。但是，**ESSID 为空并不能说明是隐藏 SSID，ESSID 不为空也不能说明其不是隐藏 SSID。i.e. 我并没有找到一种判断 SSID 是否是隐藏 SSID 的决定性方法。**但是，还是有一种笨方法：规定一段时间，记录下所有发送 Beacon 的 SSID 取并集，并将该段时间所抓到的数据包用 aircrack 呈现，通过比较得到哪些 SSID 是隐藏 SSID。(个人感觉这种方法不靠谱......)
...
==[这里默认从抓包的结果分析手机WLAN 扫描结果]==    从理论上分析，手机在抓包时间窗口内在手机端的无线网络列表可以看到的SSID，应该是**该时间段内发送过Beacon 数据包且数据包被手机成功接受的 AP 的 SSID**。但实际情况可能会更复杂，这里提供几个可以考虑的点 **(1) 抓包设备 和 手机 无限信号的接收范围 (2) Beacon 帧发送时的时间间隔  (3) chancel hopping 影响**。

我稍微查了一下 问题（2），问题（2）可以被分解为以下问题：

**[1]** Beacon 帧的发送是否是连续的？

**[2]** 手机 WLAN 列表 接受 Beacon 帧的过程是否是连续的？
...
Probe Request （同理 Probe Reponse，这里就不再截图）（管理帧）
